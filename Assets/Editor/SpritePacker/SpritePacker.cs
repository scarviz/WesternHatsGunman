...
public class SpritePacker : EditorWindow
{
	...
	void UpdateParent(Transform parent)
	{
		...
		int i0 = 0;
		foreach (Renderer r in theRenderers) { renderers[i0] = r; i0++; }
		i0 = 0;
		foreach (Animation a in theAnimations) { animations[i0] = a; i0++; }
		i0 = 0;
		foreach (SpritePackerAnimation s in theProviders) { providers[i0] = s; i0++; }
		i0 = 0;
		foreach (ParticleSystem p in theParticleSystems) { particleSystems[i0] = p; i0++; }
		i0 = 0;
